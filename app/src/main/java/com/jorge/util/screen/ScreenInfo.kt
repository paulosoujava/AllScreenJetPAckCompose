...
class ScreenInfo {
    ...
    fun createClassifier(devicePosture: WindowLayoutInfo, windowDpSize: DpSize): ScreenClassifier {
        ...
        return if (foldingFeature == null) {
            ...
            createTableTopMode(foldingFeature)
        } else {
            createFullyOpenedDevice(
                windowDpSize
            )
        }
    }

    private fun createTableTopMode(foldingFeature: FoldingFeature): ScreenClassifier.HalfOpened.TableTopMode {
        ...
        return ScreenClassifier.HalfOpened.TableTopMode(
            hingPosition = foldingFeature.bounds,
            hingeSeparationRatio = ratio,
            isSeparating = foldingFeature.isSeparating,
            occlusionTye = foldingFeature.occlusionType
        )
    }
